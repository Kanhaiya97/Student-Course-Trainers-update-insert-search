...
class DB {
    ...
    function __construct(){
        $this->username = 'root';
        $this->password = '';
        $this->databasename = 'coding_kanhaiya';
    ...
    function get_courses(){ //all courses
        ...
        $sql = "SELECT a.id, a.course_name, a.course_fee, a.description, a.course_image, b.name, b.domain FROM courses a
         INNER JOIN trainers b ON a.trainer_id = b.name";
        $result = mysqli_query($this->connection, $sql);
        if($result == true){
            $data=[];
            while($row = mysqli_fetch_assoc($result)){
    ...
    function admin_login($email, $password){
        ...
        $sql = "SELECT * FROM users WHERE email='$email' and password=sha1('$password') and role=9"; //if i will not give  and role =9 than i can login guest/admin/resisterd what i want 
        $result = mysqli_query($this->connection,$sql);
        //$row = mysqli_fetch_assoc($result);
